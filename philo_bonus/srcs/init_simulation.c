...
/*   Updated: 2021/08/17 21:15:57 by nlecaill         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
*/
static int	semaphores_initialisation(t_philo_simu *simu, char *sem_names[4])
{
	int		i;
	int		sem_value;
...
int	init_simulation_next(t_philo_simu *const simu)
{
	...
	sem_names[3] = "writing";
	err = semaphores_initialisation(simu, sem_names);
	if (err != SUCCESS)
		return (err);
	game_info(simu);
	return (SUCCESS);
...
int	init_simulation(int ac, char **av, t_philo_simu *const simu)
{
	...
	(*simu) = (t_philo_simu){ft_atoi(av[1]),
		ft_atoi(av[2]),
		ft_atoi(av[3]),
		ft_atoi(av[4]),
		-1,
